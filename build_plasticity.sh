...
TARGET_DEB_URL=https://github.com/nkallen/plasticity/releases/download/v25.3.8/plasticity_25.3.8_amd64.deb
...
# extract deb files (using ar and tar instead of dpkg)
mkdir -p "${TEMP_DIR}/DebExtract"
cd "${TEMP_DIR}/DebExtract"
ar x "${TEMP_DIR}/plasticity.deb"
mkdir -p "${TEMP_DIR}/DebFiles"
tar xf data.tar.* -C "${TEMP_DIR}/DebFiles"
cd -

# install plasticity binary to /usr/bin so ldd can find it
sudo cp -a "${TEMP_DIR}/DebFiles/usr/"* /usr/
